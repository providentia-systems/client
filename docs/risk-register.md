...
| F-13 | A composed catalog route is mistaken for accepted end-to-end evidence | Permission-gated consent, separately confirmed per-item identity submission, validated revision-bound icon metadata, and the platform-role workbench use privacy allowlists; permission/role/session loss clears protected state | Live backend catalog acceptance and evidence for every enabled contribution category |
