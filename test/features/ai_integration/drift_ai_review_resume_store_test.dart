...
void main() {
  ...
  test(
    'resume IDs are durable, idempotent and account/home isolated without cached evidence',
    () async {
      final alice = DriftAiReviewResumeStore(
        database,
        homeId: _home,
        accountId: _alice,
      );
      final bob = DriftAiReviewResumeStore(
        database,
        homeId: _home,
        accountId: _bob,
      );
      const reference = AiReviewResumeReference(
        extractionId: _extract,
      ...
      expect(
        await DriftAiReviewResumeStore(
          database,
          homeId: _bob,
          accountId: _alice,
        ).list(),
  ...
  test(
    'corrupt, unbound, foreign and unknown-kind hints cannot become extraction requests',
    () async {
      final store = DriftAiReviewResumeStore(
        database,
        homeId: _home,
        accountId: _alice,
      );
      ...
      final payloads = [
        '{',
        '[]',
        jsonEncode({...valid, 'accountId': _bob}),
        jsonEncode({...valid, 'extractionId': 'invalid'}),
        jsonEncode(valid),
      ];
        ...
        await database
            ...
            .insert(
              LocalRecordsCompanion.insert(
                homeId: _home,
                entityType: ClientLocalRecordTypes.aiReviewResume,
                entityId: 'unbound-$i',
                payload: payloads[i],
      ...
      await database
          ...
          .insert(
            LocalRecordsCompanion.insert(
              ...
              payload: jsonEncode({...valid, 'kind': 'unknown'}),
              updatedAt: DateTime.now().toUtc(),
            ),
          );
  ...
  test(
    'recent review list is bounded without losing original stored references',
    () async {
      final store = DriftAiReviewResumeStore(
        database,
        homeId: _home,
  ...
  test(
    'invalid scope and extraction identifiers fail before persistence',
    () async {
      expect(
        () => DriftAiReviewResumeStore(database, homeId: '', accountId: _alice),
        throwsArgumentError,
      );
      expect(
        () => DriftAiReviewResumeStore(database, homeId: _home, accountId: ''),
        throwsArgumentError,
      );
      final store = DriftAiReviewResumeStore(
        database,
