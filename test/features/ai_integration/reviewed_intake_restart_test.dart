...
void main() {
  for (final decision in AiObservationDecision.values) {
    test('accepted candidates cannot bypass $decision evidence', () {
      ...
      if (decision == AiObservationDecision.distinct) {
        expect(
          const AiReviewHandoffBuilder().build(review).acceptedCandidates,
          hasLength(2),
        );
      } else {
        expect(
          () => const AiReviewHandoffBuilder().build(review),
  ...
  test(
    'rejected discrepancy prevents handoff even after candidate acceptance',
    () {
      final review = _review(
        discrepancies: const [
          AiDiscrepancyReview(
            ...
            primary: 'A',
            validation: 'B',
            decision: AiDiscrepancyDecision.rejectedExtraction,
          ),
        ],
      );
      expect(
  ...
  test(
    'receipt intake survives database reopen and preserves manual edits',
    () async {
      ...
      try {
        ...
        var controller = ReceiptAiHandoffController(
          handoff: handoff,
        ...
        await repository.updateReceiptDraftLine(
          lineId: capture.lines.first.id,
          expectedRevision: capture.lines.first.revision,
          line: PurchaseReceiptLineRequest(
            homeId: _home,
            receiptId: capture.id,
            rawDescription: 'Manually corrected rice',
            quantity: 3,
            lineTotal: Money(minorUnits: 300, currency: 'NAD'),
          ),
        );
        ...
        controller = ReceiptAiHandoffController(
          handoff: handoff,
          repository: repository,
          activeHomeId: _home,
          mayWritePurchases: true,
        ...
        final resumed = (await repository
            .watchActiveReceiptCapture(homeId: _home)
            .first)!;
...
AiExtractionReview _review({
  AiObservationDecision decision = AiObservationDecision.distinct,
  List<AiDiscrepancyReview> discrepancies = const [],
}) => AiExtractionReview(
  homeId: _home,
  extractionId: _extraction,
  ...
  observations: [
    AiObservationReview(
      ...
      revision: 1,
      exactDigest: false,
      leftReference: 'observation:0:candidate:0',
      rightReference: 'observation:1:candidate:0',
      leftCandidatePosition: 0,
      rightCandidatePosition: 1,
      decision: decision,
    ),
  ],
  ...
  candidates: [
    for (var position = 0; position < 2; position++)
      AiReviewCandidate(
        ...
        label: 'Rice',
        status: AiCandidateReviewStatus.accepted,
        revision: 2,
        receiptPayload: AiReceiptCandidatePayload(
          rawText: 'Original scan line $position',
          description: 'Rice',
          quantity: 1,
          packText: null,
          unitPriceMinorUnits: 100,
          lineTotalMinorUnits: 100,
          header: AiReceiptHeaderPayload(
