...
void main() {
  for (final status in [SyncRunStatus.completed, SyncRunStatus.uploadsPending, SyncRunStatus.uploadsBlocked]) {
    test('category refresh failure preserves $status upload evidence and prior caches', () async {
      final delegate = _Sync(SyncRunOutcome(status: status, pullCompleted: true,
        remainingUploads: status == SyncRunStatus.completed ? 0 : 2,
        acknowledgedCount: 1, pulledChangeCount: 3,
      ));
      var writes = 0;
      final sync = ItemMasterRefreshingSynchronization(
        delegate: delegate, homeId: 'home', source: const _Items(),
        categorySource: const _Categories(fail: true),
        replaceCache: ({required homeId, required items, categories}) async { writes++; },
      );
      final outcome = await sync.synchronize('home');
      expect(writes, 0);
      expect(outcome.status, status == SyncRunStatus.completed ? SyncRunStatus.retryableFailure : status);
      ...
      if (status != SyncRunStatus.completed) {
        expect(outcome.safeMessage, isNot(contains('Home changes synchronized')));
      }
    });
  }
  test('complete published category snapshot reaches the atomic cache writer with item master', () async {
    var writes = 0;
    final sync = ItemMasterRefreshingSynchronization(
      delegate: const _Sync(SyncRunOutcome(status: SyncRunStatus.completed)),
      homeId: 'home', source: const _Items(), categorySource: const _Categories(),
...
final class _Categories implements PublishedCategorySource {
  ...
  Future<List<PublishedInventoryCategory>> loadAll() async {
    ...
    return [PublishedInventoryCategory(id: 'global', name: 'Produce', revision: 1)];
  }
}
final class _Sync implements AppSynchronization {
  ...
  @override
  Stream<SyncSummary> watchSummary({required String homeId}) => const Stream.empty();
  @override
  Future<ConnectivityResult> connectivity() async => const ConnectivityResult(availability: SyncAvailability.online);
}
