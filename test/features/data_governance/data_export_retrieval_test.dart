...
void main() {
  for (final scope in DataGovernanceScope.values) {
    test(
      'retrieves ${scope.name} through body-only token and erases memory',
      () async {
        ...
        final artifact = await repository.retrieveExport(
          _request(scope),
          isCurrent: () => true,
        );
  ...
  test(
    'refreshes revision after a consumed token, never reuses that token',
    () async {
      var reads = 0;
      var issued = 0;
  ...
  for (final failure in [
    ...
    test(
      'fails closed for $failure without exposing private error detail',
      () async {
        var current = true;
        var downloads = 0;
        final repository = _repository((request) async {
          ...
          if (request.url.path.endsWith('/download-token')) {
            ...
            return _json({
              ...
              'revision': 8,
              'expiresAt': _future,
            }, noStore: failure != 'cacheable-token');
          }
          downloads++;
        ...
        await expectLater(
          repository.retrieveExport(
            _request(DataGovernanceScope.home),
            isCurrent: () => current,
          ),
          throwsA(
        ...
        expect(
          downloads,
          failure == 'wrong-home' || failure == 'cacheable-artifact' ? 1 : 0,
        );
      },
    );
  }
...
GeneratedDataGovernanceRepository _repository(
  Future<http.Response> Function(http.Request) handle,
) => GeneratedDataGovernanceRepository(
  ProvidentiaApiClient(
    baseUri: Uri.parse('https://api.example.test'),
    httpClient: MockClient(handle),
  ),
);
http.Response _json(Object body, {int status = 200, bool noStore = true}) =>
    http.Response(
      jsonEncode(body),
      status,
      headers: {
        'content-type': 'application/json',
        if (noStore) 'cache-control': 'private, no-store',
      },
    );
Map<String, Object?> _row(DataGovernanceScope scope, {int revision = 7}) => {
  'id': _id,
  'scopeType': scope.name,
  'requestKind': '${scope.name}_export',
  'homeId': scope == DataGovernanceScope.home ? _home : null,
  'status': 'completed',
  'revision': revision,
  'downloadEligible': true,
  'artifactExpiresAt': _future,
  'retainedDataDisclosure': <Object?>[],
};
DataGovernanceRequest _request(DataGovernanceScope scope) =>
    DataGovernanceRequest(
      ...
      homeId: scope == DataGovernanceScope.home ? _home : null,
      downloadEligible: true,
      artifactExpiresAt: DateTime.parse(_future),
      retainedDataDisclosure: [],
    );
Map<String, Object?> _artifact(DataGovernanceScope scope) => {
  'format': 'providentia-data-export-v1',
  'requestId': _id,
  'scope': scope.name,
  'data': scope == DataGovernanceScope.home
      ? {
          'home': [
            {'id': _home},
          ],
