...
void main() {
  for (final result in DataExportSaveResult.values) {
    testWidgets(
      'actual export page requires explicit save and handles ${result.name}',
      (tester) async {
        final repository = _Repository();
        final saver = _Saver(result: result);
        final controller = DataGovernanceController(
          _service(repository),
          exportSaver: saver,
        );
        ...
        expect(
          find.textContaining('synthetic-sensitive-payload'),
          findsNothing,
        );
  ...
  for (final dispose in [false, true]) {
    test(
      'late download after ${dispose ? 'disposal' : 'account switch'} cannot expose bytes',
      () async {
  ...
  test(
    'discard and failing native save remove private memory and preserve safe notices',
    () async {
      final repository = _Repository();
      final saver = _Saver(
        error: StateError('synthetic-sensitive-payload /private/path'),
      );
      final controller = DataGovernanceController(
        _service(repository),
        exportSaver: saver,
  ...
  test(
    'unsupported platform fails closed without keeping downloaded content',
    () async {
      final repository = _Repository();
  ...
  testWidgets('artifact expiry clears a ready download before save', (
    tester,
  ) async {
  ...
  for (final kind in DataGovernanceFailureKind.values) {
    test('download classifies ${kind.name} without raw details', () async {
      final controller = DataGovernanceController(
        _service(_Repository(error: DataGovernanceRepositoryException(kind))),
      );
      addTearDown(controller.dispose);
      await controller.download(_request());
      expect(controller.status, DataGovernanceViewStatus.failure);
  ...
  test(
    'service rejects foreign home, erasure and invalidated session before retrieval',
    () {
      final repository = _Repository();
      final service = _service(repository);
      expect(
        () => service.retrieveExport(
          _request(homeId: _otherHome),
          isCurrent: () => true,
        ),
      ...
      expect(
        () => service.retrieveExport(
          _request(erasure: true),
          isCurrent: () => true,
        ),
        throwsA(isA<DataGovernanceCapabilityException>()),
      );
      expect(
        () => service.retrieveExport(_request(), isCurrent: () => false),
        throwsA(isA<DataGovernanceCapabilityException>()),
      );
...
DataGovernanceService _service(_Repository repository) => DataGovernanceService(
  repository: repository,
  capabilities: DataGovernanceCapabilities.fromEffectivePermissions(
    authenticated: true,
    effectiveHomePermissions: const {HomePermissions.dataExport},
  ),
  activeHomeId: _home,
);
DataGovernanceRequest _request({String? homeId, bool erasure = false}) =>
    DataGovernanceRequest(
      ...
      artifactExpiresAt: DateTime.now().toUtc().add(const Duration(hours: 1)),
      retainedDataDisclosure: [],
    );
DataExportArtifact _artifact({Duration lifetime = const Duration(hours: 1)}) =>
    DataExportArtifact(
      requestId: _requestId,
      scope: DataGovernanceScope.account,
      expiresAt: DateTime.now().toUtc().add(lifetime),
      bytes: Uint8List.fromList('synthetic-sensitive-payload'.codeUnits),
    );
...
final class _Repository
  ...
  dynamic noSuchMethod(Invocation invocation) =>
      throw UnsupportedError('Unexpected mutation: ${invocation.memberName}');
}
